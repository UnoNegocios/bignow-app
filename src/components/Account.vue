<template>
    <v-container class="pa-0">
        <v-row class="ma-0 py-2 px-2">
            ...
            <v-spacer/>
            <v-btn @click="save()" :disabled="disabled_button" :loading="loader_button" color="#e12826" class="mt-1" small text>guardar</v-btn>
        </v-row>

        <v-row class="ma-0 my-6" id="account">
            <v-spacer/>
        ...
        <div class="mt-12 pt-12 px-8" style="text-align:center; height:calc(100vh - 225px)!important; background:white; border-radius: 10px 10px 0px 0px; box-shadow: 0px 10px 20px 10px #00000018!important;">
            ...
            <v-textarea v-model="currentUser.address" label="Dirección" outlined style="border-radius:5px;" dense></v-textarea>
            <v-row class="my-1">
                <v-spacer/>
                <v-dialog v-model="dialog2" width="325">
                    ...
                    <v-card class="pa-6">
                        ...
                        <v-row class="ma-0 mt-6">
                            <v-spacer/>
                            <v-btn type="submit" @click="savePassword()" class="py-3 elevation-0" color="#e12826" :dark="this.min8(this.password1) === true && this.matchingPasswords() === true" rounded :disabled="!(this.min8(this.password1) === true && this.matchingPasswords() === true)"><strong>Guardar</strong></v-btn>
                        </v-row>    
                    </v-card>
                </v-dialog>
                <v-spacer/>
            </v-row>
            <v-btn @click="logout()" bottom absolute style="margin-bottom:50px; margin-left:calc(50vw - 100px); color:grey;" left class="elevation-0" color="white">
...
#account .theme--dark.v-badge .v-badge__badge::after {
    border-color: #eeeeee;
}
